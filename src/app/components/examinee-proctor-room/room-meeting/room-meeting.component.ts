...
export class RoomMeetingComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.utility.roomEdit$.subscribe(data => {
      if (this.listRoom) {
        let room = this.listRoom.find(x => x.examId === data.examId);
        if (room)
    ...
    this.utility.roomCount$.subscribe(data => {
      if (this.listRoom) {
        let room = this.listRoom.find(r => r.examId === data.examId)
        if (room)
  ...
  deleteRoomById(id: number) {
    this.confirmService.confirm().subscribe(value => {
      if (value) {//true
        this.roomService.deleteExam(id).subscribe(res => {
          if (res) {
            this.loadRooms();
          }
        })
  ...
  ngChanged(e) {
    //console.log(e)
    this.listRoom = this.dataGroupByUser.get(e.userName)//userName key
  }

  joinRoom(examId: number) {
    this.router.navigate(['/home', examId])
  }
